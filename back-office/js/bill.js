...
const getBills = async (query) => {
  const response = await fetch("http://site212222.tw.cs.unibo.it/api/v1/bill?" + new URLSearchParams(query));
  if (!response.ok) {
...
const getBill = async (id) => {
  if (id) {
    const response = await fetch(`http://site212222.tw.cs.unibo.it/api/v1/bill/${id}`);
    if (!response.ok) {
      const error = await response.json();
      const errorTemplate = Handlebars.compile($("#errorTemplate").html());
      const filled = errorTemplate({ error: error.msg });
