...
const isAuth = require(global.rootDir + '/routes/authMiddleware.js').isAuth;
...
router.post('/register', (req, res) => {
    ...
    if (req.body.password.length < 8) { errors.push('La password deve avere almeno 8 caratteri') }
    if (!validPass.test(req.body.password)) { errors.push('La password deve contenere almeno una lettera maiuscola, una minuscola, una cifra e un carattere speciale') }
    bcrypt.hash(req.body.password, 10, (err, hash) => {
        User.findOne({ email: req.body.email })
            .then((user) => {
                if (user) { errors.push('Email già registrata') }
                if (errors.length > 0) {
